...
export function Bonuses() {
  return (
    <section id="bonuslar" className="border-b border-soft bg-[#0a0420]">
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
        <div className="flex items-end justify-between gap-3 border-b border-soft pb-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-[15px] leading-7 text-[#cfc6ef]">
              Mekanbahis hem yeni üyeleri hem de mevcut kullanıcıları çeşitli
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {BONUSES.map((b) => (
            ...
            >
              <span className="inline-flex w-fit rounded-full border border-pink/30 bg-pink/10 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wider text-[#ff7ab8]">
                {b.tag}
              </span>
              <div className="font-display mt-4 text-4xl font-extrabold tracking-tight text-gradient-neon">
                {b.amount}
              </div>
              <div className="mt-1 text-xs text-[#b0a7d6]">{b.cap}</div>
              <h3 className="font-display mt-4 text-base font-bold text-white">
                {b.title}
              </h3>
              <p className="mt-2 flex-1 text-sm leading-6 text-[#cfc6ef]">
                {b.desc}
              </p>
              <a
                ...
                className="mt-5 inline-flex h-10 cursor-pointer items-center justify-center rounded-md border border-pink/30 bg-pink/10 px-4 text-xs font-semibold text-[#ff7ab8] transition-colors hover:border-pink/60 hover:bg-pink/20"
              >
                Bonusu Talep Et
              </a>
            </article>
