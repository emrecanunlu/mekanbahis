  ...
  if (state?.ok) {
    return (
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-[#cfc6ef]">{state.message}</p>
      </div>
    );
  }
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <button
        type="submit"
        disabled={pending}
        className="inline-flex h-12 w-full cursor-pointer items-center justify-center gap-2 rounded-md bg-gradient-to-r from-pink-500 to-fuchsia-600 px-6 text-sm font-semibold text-white shadow-[0_10px_30px_-10px_rgba(232,62,140,0.55)] transition-transform hover:from-pink-400 hover:to-fuchsia-500 disabled:cursor-not-allowed disabled:opacity-60 active:translate-y-px sm:w-auto"
      >
        {pending ? (
          <>
            <Spinner />
...
function Field({
  ...
  return (
    <div>
      <label
        htmlFor={htmlFor}
        className="mb-1.5 block text-sm font-medium text-[#cfc6ef]"
      >
        {label}
...
const inputCls =
  "w-full rounded-xl border border-soft bg-[#0a0420] px-4 py-3 text-sm text-white placeholder:text-[#7e76a8] outline-none transition-colors focus:border-pink/50 focus:ring-2 focus:ring-pink/20";
