    ...
    <section className="space-y-5">
      {showHeader && (
        <div className="flex items-end justify-between gap-3 border-b border-soft pb-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[#b0a7d6]">
              Mekanbahis hakkında rehberler, kampanya duyuruları ve güncel
              gelişmeler.
            </p>
          </div>
          {limit && (
            <Link
              href="/blog"
              className="hidden shrink-0 rounded-md border border-soft px-3 py-1.5 text-xs font-medium text-[#cfc6ef] transition-colors hover:border-pink/40 hover:text-pink sm:inline-flex"
            >
              Tümünü Gör
            </Link>
          )}
      ...
      {limit && (
        <div className="pt-2 sm:hidden">
          <Link
            href="/blog"
            className="inline-flex rounded-md border border-soft px-3 py-1.5 text-xs font-medium text-[#cfc6ef] transition-colors hover:border-pink/40 hover:text-pink"
          >
            Tüm Yazıları Gör
          </Link>
        </div>
...
export function PostCard({ post }: { post: Post }) {
  return (
    ...
    >
      <div className="flex items-center gap-2 text-xs text-[#b0a7d6]">
        <span className="rounded-full border border-pink/30 bg-pink/10 px-2 py-0.5 font-medium text-[#ff7ab8]">
          {post.category}
        </span>
      ...
      <p className="mt-2 line-clamp-3 flex-1 text-sm leading-6 text-[#cfc6ef]">
        {post.excerpt}
      </p>
